<script setup>
// Ports executive_code_logs/index.blade.php ("Executive Code Logs") —
// identical structure to leader_code_logs/index.blade.php with
// "Executive" substituted for "Leader" throughout, per ui_spec.md. See
// user_handler.go's codeLogsHandler("executive_code_logs", ...).
...
async function fetchData(page = 1) {
  ...
  try {
    ...
    const { data } = await api.get('/executive-code-logs', { params })
    logs.value = data.logs
  } catch (err) {
    loadError.value = err?.response?.data?.message || 'Could not load executive code logs.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <DashboardLayout>
    ...
    <div class="py-4 d-flex align-items-center justify-content-between">
      <h1 class="h4 mb-0">Executive Code Logs</h1>
          ...
          <table class="table align-items-center table-flush">
            <thead class="thead-light">
              <tr>
                ...
                <th class="border-bottom">Old Executive</th>
                <th class="border-bottom">New Executive</th>
            ...
            <tbody>
              <tr v-if="!loading && !logs?.data?.length">
                <td colspan="5" class="text-center text-muted py-4">No executive changes found</td>
              </tr>
